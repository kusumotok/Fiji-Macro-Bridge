...
1. GitHub Releases から `fiji-macro-bridge-windows-x64.zip` をダウンロードして展開する
2. `install.bat` を実行する
3. Claude Desktop などの MCP client を再起動する
4. 接続テストとして Fiji を開いた状態で `launch_fiji` を呼ぶ
5. 必要なら `%LOCALAPPDATA%\FijiMacroBridge\uninstall.bat` でアンインストールする
...
生成された `plugin/target/Fiji_Macro_Bridge.jar` を Fiji の `plugins/` に配置してください。
